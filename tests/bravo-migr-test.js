...
const {newUser, fetchJson, existingUser, callFioApiSigned, callFioApi} = require('../utils.js');
...
describe(`Confirm new OBT Sends are going into both tables`, () => {
    let user1, user2, user3
    let payment = 3000000000;
...
describe(`Confirm NEW Requests are going into both tables`, () => {
...
describe.skip(`Migrate remaining requests and OBTs`, () => {
    ...
    it(`Call migrtrx with amount = ${amount} 100 times until return code (bp1)`, async () => {
        for (i = 0; i < 100; i++) {
            try {
                const result = await callFioApiSigned('push_transaction', {
                    action: 'migrtrx',
