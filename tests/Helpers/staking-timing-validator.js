...
class StakingPeriodValidator {
  ...
  async getUserBalance (user) {
     ...
     */
    try {
      return await user.sdk.genericAction('getFioBalance', {});
    } catch (err) {
  ...
  async validateStakingBalances (user, balanceObj, expBalance, expAvailable, expStaked, expSrps) {
    /**
     ...
     * @param {Object} user a user object
     * @param {Object} balanceObj a balance object from getFioBalance
     ...
     */
    // let balObj = await this.getUserBalance(user);  //user.sdk.genericAction('getFioBalance', {});
...
module.exports = StakingPeriodValidator;
