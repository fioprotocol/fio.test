...
describe(`************************** tpid.js ************************** \n TPID Tests`, () => {
  ...
  it(`NOT APPLICABLE: setdomainpub: user2 tries to make user4.domain public`, async () => {
    try{
      const result = await user2.sdk.genericAction('pushTransaction', {
        action: 'setdomainpub',
        account: 'fio.address',
        data: {
          fio_domain: user4.domain,
          is_public: 1,
          max_fee: config.maxFee,
          tpid: proxy1.address,
          actor: user2.account
      ...
      expect(err.json.fields[0].error).to.equal(config.error.actorNotDomainOwner);
    }
  })

  it(`BUG: regaddress: user3 register new Address for user1 using FIO Address of proxy1 (who IS a registered proxy). Expect: no proxy for user1.`, async () => {
  ...
  it.skip(`TODO: xferaddress: user3 transfers user3.address2 to emptyAccount.publicKey using FIO Address of proxy1 (who IS a registered proxy). Expect: no proxy for user1.`, async () => {
    try{
        const result = await user3.sdk.genericAction('pushTransaction', {
            action: 'xferaddress',
            account: 'fio.address',
  ...
  it(`BUG: trnsfiopubky: user 3 transfers 10 FIO to user1 using FIO Address of proxy1 (who IS a registered proxy). Expect: no proxy for user1.`, async () => {
    try {
      const result = await user3.sdk.genericAction('pushTransaction', {
        action: 'trnsfiopubky',
        account: 'fio.token',
