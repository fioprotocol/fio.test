...
/*
  ...
  2) reallocation account has in-adequate balance.
  3) reallocation account has no locked tokens
  4) receiving account not on chain
  5) receiveing account has no locks
 */

//expected result
//assertion failure with message: fip48 NO WORK PERFORMED account has no lockedtokens table entry xkezj1ocwe4r
describe.skip(`Error (fresh chain start required) reallocation account does not exist. Setup error case on local test box, verify expected error.`, () => {
  ...
  it.skip(`Lock  tokens account1 lock type 2`, async () => {
...
Error Details:
Error code 400
: {"type":"invalid_input","message":"An invalid request was sent in, please check the nested errors for details.","fields":[{"name":"fip48 token transfer","value":"10000000000000000","error":"Insufficient funds to cover fip48 transfer mck32myftiau"}]}

 */
describe.skip(`Error (fresh chain start required) reallocation account inadequate balance. Setup error case on local test box, verify expected error.`, () => {
  ...
  //SETUP ACCOUNT 2 with inadequate balance.
  const account2Info = {
    account: 'mck32myftiau',
    publicKey: 'FIO7DVJTbCjmBev9oAggRP7kRjXJfZAE4228SnMfsyyBhv7wggxDB',
    fioAmount: 9500000000000000
  }
  const account3Info = {
    account: 'hjvwdy5p4zvs',
    publicKey: 'FIO7BQfxKtfebgrTZYbMYi6gvcWZwU8dogL78jAwNgeQVn5xazKAv',
...
describe.skip(`Error (fresh chain start required) reallocation account no locked tokens. Setup error case on local test box, verify expected error.`, () => {
  ...
  //no locked tokens for account 2, failure test setup.
  it.skip(`Lock  tokens account2 lock type 2`, async () => {
    const result1 = await userA1.sdk.genericAction('pushTransaction', {
      action: 'addlocked1',
      account: 'eosio',
      data: {
 ...
 */
describe.skip(`Error (fresh chain start required) receiving account does not exist. Setup error case on local test box, verify expected error.`, () => {
  ...
  //test receiver account missing on chain
  it.skip(`Transfer 1 token to receiver account`, async () => {
    const result = await faucet.genericAction('transferTokens', {
      payeeFioPublicKey: receiveraccountInfo.publicKey,
      amount: 1000000000,
      maxFee: config.api.transfer_tokens_pub_key.fee,
  ...
  it.skip(`Lock 1000000000000000 tokens receiver account lock type 4 remaining 0`, async () => {
    const result1 = await userA1.sdk.genericAction('pushTransaction', {
      action: 'addlocked1',
      account: 'eosio',
      data: {
...
describe.only(`Error (fresh chain start required) receiving account has no locked tokens. Setup error case on local test box, verify expected error.`, () => {
  ...
  //receiving account no locked tokens.
  it.skip(`Lock 1000000000000000 tokens receiver account lock type 4 remaining 0`, async () => {
    const result1 = await userA1.sdk.genericAction('pushTransaction', {
      action: 'addlocked1',
      account: 'eosio',
