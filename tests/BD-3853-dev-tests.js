...
describe(`************************** BD-3853-dev-tests.js ************************** \n    A. Stake from small balance of tokens before and after voteproducer.`, () => {
  ...
  it(`Set up users`, async () => {
    ...
    console.log('vpfee: ', config.api.vote_producer.fee)
  })
  ...
  it(`(Bug BD-3853 Unstake should fail since no fees available) Failure -- sdkstaker unstakes all staked FIO. Expect error: Insufficient funds to cover fee )`, async () => {
    try {
      const result = await sdkStaker.genericAction('pushTransaction', {
        action: 'unstakefio',
        account: 'fio.staking',
...
describe(`B. Stake from small balance of tokens before and after voteproducer.`, () => {
